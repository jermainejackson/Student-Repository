...
class RepositoryUnitTest(unittest.TestCase):
    ...
    def test_student_repository(self) -> None:
        ...
        self.assertEqual(sorted(list((set(self.university_data.major_file_reader()[('SFEN', 'R')])
                    - set(self.university_data.students_data.files_summary_student['10103']['Completed Courses'])))), ['SSW 540', 'SSW 555'])


        self.assertEqual([x for x in (list(set(self.university_data.major_file_reader()[('SFEN', 'E')])
                - set(self.university_data.students_data.files_summary_student['10103']['Completed Courses'])))
               if not set(self.university_data.major_file_reader()[('SFEN', 'E')]).intersection(set(self.university_data.students_data.files_summary_student['10103']['Completed Courses']))
               ],[])

        self.assertEqual(sorted([x for x in (list(set(self.university_data.major_file_reader()[('SFEN', 'E')])
                - set(self.university_data.students_data.files_summary_student['10172']['Completed Courses'])))
               if not set(self.university_data.major_file_reader()[('SFEN', 'E')]).intersection(set(self.university_data.students_data.files_summary_student['10172']['Completed Courses']))
